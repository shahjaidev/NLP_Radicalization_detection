...
#------------------------------------------------------------------------------------------------------------------------------------
    ...
    def top_k_most_relevant(self, recos, current_video_id, lim=2):
        ...
        myVideo = YouTube("https://www.youtube.com/watch?v=" + current_video_id)
        cur_title=myVideo.title


        for video_id in recos:
            reco_video_object= YouTube("https://www.youtube.com/watch?v=" + video_id)
            reco_title= reco_video_object.title
        ...
        tfidf_vectorizer_vectors=tfidf_vectorizer.fit_transform(title_list)
        i=0

        print(tfidf_vectorizer_vectors[0])
        tfidf_vectors={}
        for k in sorted(titles.keys()):
            tfidf_vectors[k]= tfidf_vectorizer_vectors[i]
            i=i+1


        #cur_title_vector = pd.DataFrame(first_vector_tfidfvectorizer.T.todense(), index=tfidf_vectorizer.get_feature_names(), columns=["tfidf"])
        

        for video_id in sorted(titles.keys()):
            scores[video_id]= cosine_similarity(tfidf_vectors[cur_title], tfidf_vectors[video_id])
        
        #spacy
        """
        cur_title_spacy= nlp(cur_title)

        scores={}

        for video_id in sorted(titles.keys()):
            video_title_spacy= nlp(titles[video_id])
            scores[video_id]= cur_title_spacy.similarity(video_title_spacy)
