...
class YoutubeFollower():
    ...
    def get_recommendations(self, video_id, nb_recos_wanted, depth, key):
        ...
        top_3_recs= self.top_k_most_relevant(recos[:10], video_id, 3) #passing in only first 10 videos on sidebar
        print("hangg")
        video = self._video_infos[video_id]
        ...
        return top_3_recs[:nb_recos_wanted]
...
#------------------------------------------------------------------------------------------------------------------------------------
    ...
    def follow_sequence(self, video_id, branching, depth):
        ...
        while(depth>0):
            print(video_id)
            top_3_recos_in_sidebar= self.get_recommendations_modified(video_id, branching, depth)
            print("Printing top 3")
            print(top_3_recos_in_sidebar)
            next_vid_idx= random.randint(0,2)

            video_id= top_3_recos_in_sidebar[next_vid_idx]
            depth=depth-1
        return
    ...
    def top_k_most_relevant(self, recos, current_video, k=3):
        titles={}
        print(recos)
        cur_video= pafy.new(current_video)
        cur_title= pafy_video.title 


        for video_id in recos:
            reco_video = pafy.new(video_id)
            reco_title= reco_video.title
        ...
        with tf.Session() as session:
            ...
            for video_id in recos:
                messages=[cur_title, titles[video_id] ]
                message_embeddings = session.run(embed(messages))
                print(message_embeddings[1])
